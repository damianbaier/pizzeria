const pizzas = [
  {
    id: 1,
    nombre: "pizza de Muzzarella",
    precio: 500,
    ingredientes: ["Muzzarella", "Tomate", "Aceitunas"],
  },

  {
    id: 2,
    nombre: "pizza de Cebolla",
    precio: 1500,
    ingredientes: ["Muzzarella", "Tomate", "Cebolla"],
  },

  {
    id: 3,
    nombre: "pizza Napolitana",
    precio: 1350,
    ingredientes: ["Muzzarella", "Tomate", "Aceitunas", "Anchoas"],
  },

  {
    id: 4,
    nombre: "pizza 4 Quesos",
    precio: 1380,
    ingredientes: [
      "Muzzarella",
      "Tomate",
      "Queso Azul",
      "Parmesano",
      "Roquefort",
    ],
  },

  {
    id: 5,
    nombre: "pizza Especial",
    precio: 1000,
    ingredientes: ["Muzzarella", "Tomate", "Aceitunas", "Anchoas", "Cebolla"],
  },

  {
    id: 6,
    nombre: "pizza con Anana",
    precio: 600,
    ingredientes: ["Muzzarella", "Tomate", "Anana"],
  },
];

// a) Las pizzas que tengan un id impar
console.log("a) Pizzas con id impar:");
const pizzasIdImpar = pizzas.filter(pizza => pizza.id % 2 !== 0);
pizzasIdImpar.forEach(pizza => {
  console.log(`- ${pizza.nombre} (ID: ${pizza.id})`);
});

// b) Pizzas que valgan menos de $600
console.log("b) las pizzas que valen menos de $600 son:");
const pizzaMenos600 = pizzas.find(pizza => pizza.precio < 600);
if (pizzaMenos600) {
  console.log(`- ${pizzaMenos600.nombre}`);
} else {
  console.log("No se encontraron pizzas que valgan menos de $600.");
}

// c) El nombre de cada pizza con su respectivo precio
console.log("\nc) Nombre y precio de cada pizza:");
pizzas.forEach(pizza => {
  console.log(`- ${pizza.nombre}: $${pizza.precio}`);
});

// d) Todos los ingredientes de cada pizza
console.log("\nd) Estos son los ingredientes de cada pizza:");
pizzas.forEach(pizza => {
  console.log(`- ${pizza.nombre}: ${pizza.ingredientes.join(", ")}`);
});
